...
public class QueryAnalysisService {
        ...
        public QueryAnalysisResponse analyze(QueryAnalysisRequest request, String keycloakUserId) {
                ...
                List<IndexSuggestionResponse> indexSuggestions = indexAdvisorService.suggestIndexes(parsedQuery,
                                request.getSchemaId());
                ...
                if (request.getDbConnectionId() != null) {
                        executionPlanDto = runLivePlanAndAttach(history, request.getDbConnectionId(),
                                        parsedQuery.getQueryType(), warnings);
                }

                QueryParseResponse parseResponse = parserService.parseAndFormat(request.getSql(), "POSTGRESQL");
        ...
        public QueryAnalysisResponse getById(Long id) {
                ...
                QueryParseResponse parseResponse = parserService.parseAndFormat(history.getQueryText(), "POSTGRESQL");
                ...
                return QueryAnalysisResponse.builder()
                                ...
                                .parseResult(parseResponse)
                                .dbConnectionId(history.getDbConnection() != null ? history.getDbConnection().getId()
                                                : null)
                                .executionPlan(executionPlanDto)
                                .analyzedAt(history.getAnalyzedAt())
                                .build();
        ...
        private ExecutionPlanDto runLivePlanAndAttach(QueryHistory history, Long dbConnectionId, String queryType,
                ...
                try {
                        DbConnection connection = dbConnectionRepository.findById(dbConnectionId)
                                        .orElseThrow(() -> new ResourceNotFoundException("DbConnection", "id",
                                                        dbConnectionId));

                        ExecutionPlan plan = queryExecutionPlanService.runLivePlan(history, connection,
                                        history.getQueryText(), queryType);
